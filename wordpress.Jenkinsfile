pipeline {
  ...
  stages {
    stage('Docker build image') {
      steps {
        sh 'docker build -t wp2 -f wordpress.Dockerfile .'
    ...
    stage ('Tag image') {
      steps {
        sh 'docker tag wp2:latest 943696080604.dkr.ecr.us-east-1.amazonaws.com/bs-bc3:wordpress2'
    ...
    stage('Push image to ECR') {
      steps {
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'makolab_aws', variable: 'AWS_ACCESS_KEY_ID']]) {
          ...
          sh "docker push 943696080604.dkr.ecr.us-east-1.amazonaws.com/bs-bc3:wordpress2" 
